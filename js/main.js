...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderMarkers() {
        ...
        markers.forEach((m) => {
            ...
            if (m.t < timelineStart.getTime() || m.t > timelineEnd.getTime())
                return;
            const frac =
                (m.t - timelineStart.getTime()) / Math.max(1, duration);
    ...
    timeSlider.addEventListener("input", () => {
        ...
        graphs.forEach((g) => {
            if (!g.classList.contains("hidden"))
                updateChartFromData(g.dataset.graph);
        });
        ...
        // update selection overlay as well
        updateSelectionOverlay(trackRect.left + left, timeAt);
    });
